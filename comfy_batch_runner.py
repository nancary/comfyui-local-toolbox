...
- 可作为命令行工具直接跑：  python comfy_batch_runner.py --workflow wf.json --input-folder D:/图片/3
...
# 输出文件命名规则（简化版）：
#   用户在 WebUI 填写一个「文件名称前缀」(basename) 即可，例如 "myname"。
...
def download_outputs(client, entry, out_dir, index, orig_fn, basename=""):
    ...
    seq = f"{index:04d}"
    base_name = f"{basename}_{date}_{seq}" if basename else f"{date}_{seq}"
    # 去掉文件系统非法字符，保证可写
...
def scan_done_indices(output_dir):
    ...
    新命名规则下序列号位于文件名末尾：..._日期_0001.png，
    用末尾 "_{4位数字}.扩展名" 提取，避免与 8 位日期混淆。
    同时兼容旧命名（序号在文件名前缀 0001_...），保证续跑都能跳过。
    """
    done = set()
    if not os.path.isdir(output_dir):
        return done
    for fn in os.listdir(output_dir):
        m = re.search(r"_(\d{4})\.[^.]+$", fn)  # 新格式：末尾序号
        if not m:
            m = re.match(r"^(\d{4})_", fn)        # 旧格式：前缀序号
        if m:
...
def run_batch(task, is_paused=None):
    ...
    done = scan_done_indices(task["output"]) if copy_output and task["resume"] else set()
    if done:
        log_list.append(f"断点续跑：输出目录已有 {len(done)} 个结果，将跳过。")
    ...
    for i, fn in enumerate(images):
        ...
        try:
            ...
            if copy_output:
                ...
                log_list.append(f"    完成，输出 {len(saved)} 个文件")
            else:
